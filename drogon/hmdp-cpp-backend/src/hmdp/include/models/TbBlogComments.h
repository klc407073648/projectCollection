...
class TbBlogComments
{
  public:
     ...
     */
    explicit TbBlogComments(const Row &r, const ssize_t indexOffset = 0) noexcept;

    /**
     * @brief constructor
     * @param pJson The json object to construct a new instance.
     */
    explicit TbBlogComments(const Json::Value &pJson) noexcept(false);
     ...
     */
    TbBlogComments(const Json::Value &pJson, const std::vector<std::string> &pMasqueradingVector) noexcept(false);

    TbBlogComments() = default;
    ...
    /**  For column blog_id  */
    ///Get the value of the column blog_id, returns the default value if the column is null
    const uint64_t &getValueOfBlogId() const noexcept;
    ///Return a shared_ptr object pointing to the column const value, or an empty shared_ptr object if the column is null
    const std::shared_ptr<uint64_t> &getBlogId() const noexcept;

    ///Set the value of the column blog_id
    void setBlogId(const uint64_t &pBlogId) noexcept;
  ...
  private:
    friend Mapper<TbBlogComments>;
#ifdef __cpp_impl_coroutine
    friend CoroMapper<TbBlogComments>;
#endif
    ...
    std::shared_ptr<uint64_t> blogId_;
